...
class PluginHook:
    ...
    def register(self, handler: Callable) -> None:
        ...
        logger.info(
            "Plugin handler registered", hook=self.name, handler=handler.__name__
        )
...
class SimplePluginManager:
    ...
    async def load_plugin(
        self, plugin_class: Type[Plugin], config: Dict[str, Any] = None
    ) -> bool:
        ...
        try:
            ...
            logger.error(
                "Failed to load plugin", plugin=plugin_class.__name__, error=str(e)
            )
            return False
    ...
    async def discover_and_load_plugins(self, plugin_packages: List[str]) -> None:
        ...
        for package_name in plugin_packages:
            try:
                ...
                logger.debug(
                    "Plugin package not found", package=package_name, error=str(e)
                )
            except Exception as e:
                logger.error(
                    "Error loading plugin package", package=package_name, error=str(e)
                )
    ...
    async def shutdown_all_plugins(self) -> None:
        ...
        for plugin in self.plugins.values():
            try:
                ...
                logger.error("Plugin shutdown failed", plugin=plugin.name, error=str(e))
