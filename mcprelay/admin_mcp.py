...
class AdminMCPServer:
            ...
            MCPTool(
                ...
                inputSchema=MCPToolSchema(
                    properties={
                        ...
                        "weight": {"type": "number", "minimum": 0, "description": "Load balancing weight"},
                        "timeout": {"type": "number", "minimum": 1, "description": "Request timeout"},
                        "health_check": {"type": "boolean", "description": "Enable health checks"},
            ...
            MCPTool(
                ...
                inputSchema=MCPToolSchema(
                    properties={
                        ...
                        "config": {"type": "object", "description": "Updated server configuration"}
                    },
            ...
            MCPTool(
                ...
                inputSchema=MCPToolSchema(
                    properties={
                        ...
                        "is_admin": {"type": "boolean", "description": "Admin privileges"},
                        "rate_limit_tier": {"type": "string", "description": "Rate limit tier"},
                        "allowed_actions": {"type": "array", "items": {"type": "string"}, "description": "Allowed MCP actions"}
                    },
                    required=["user_id"]
                )
            ),
            ...
            MCPTool(
                ...
                inputSchema=MCPToolSchema(
                    properties={
                        ...
                        "per_user_limits": {"type": "object", "description": "Per-user rate limits"}
                    }
                )
            ),
            ...
            MCPTool(
                ...
                inputSchema=MCPToolSchema(
                    properties={
                        ...
                        "limit": {"type": "number", "minimum": 1, "maximum": 1000, "description": "Number of entries"},
    ...
    async def call_tool(
        ...
        try:
            ...
            if tool_call.name == "mcprelay_config_get":
                ...
                return await self._handle_config_update(tool_call, auth_context, request)
                ...
                return await self._handle_server_remove(tool_call, auth_context, request)
            elif tool_call.name == "mcprelay_server_update":
                return await self._handle_server_update(tool_call, auth_context, request)
                ...
                return await self._handle_rate_limits_get(tool_call, auth_context, request)
            elif tool_call.name == "mcprelay_rate_limits_update":
                return await self._handle_rate_limits_update(tool_call, auth_context, request)
            elif tool_call.name == "mcprelay_audit_logs_get":
                return await self._handle_audit_logs_get(tool_call, auth_context, request)
    ...
    async def _handle_config_get(self, tool_call: MCPToolCall, auth_context: AuthContext, request: Optional[Request]) -> MCPToolResult:
        """Handle configuration retrieval."""
        
        section = tool_call.arguments.get("section", "all")
        
        await self._log_audit_event(
            auth_context, "config_get", f"config:{section}", tool_call.arguments, "success", None, request
        )
    ...
    async def _handle_config_update(self, tool_call: MCPToolCall, auth_context: AuthContext, request: Optional[Request]) -> MCPToolResult:
        """Handle configuration updates (admin only)."""
        
        if not auth_context.is_admin:
            await self._log_audit_event(
                auth_context, "config_update", "config", tool_call.arguments, "denied", "Admin required", request
            )
        ...
        await self._log_audit_event(
            auth_context, "config_update", f"config:{section}", tool_call.arguments, "success", None, request
        )
    ...
    async def _handle_server_add(self, tool_call: MCPToolCall, auth_context: AuthContext, request: Optional[Request]) -> MCPToolResult:
        """Handle adding a new backend server."""
        
        if not auth_context.is_admin:
            await self._log_audit_event(
                auth_context, "server_add", "servers", tool_call.arguments, "denied", "Admin required", request
            )
            raise HTTPException(status_code=403, detail="Admin access required")
        ...
        await self._log_audit_event(
            auth_context, "server_add", f"server:{name}", tool_call.arguments, "success", None, request
        )
        
        return MCPToolResult(
            content=[{
                "type": "text",
                "text": f"Server '{name}' added successfully"
            }]
        )
    
    async def _handle_server_remove(self, tool_call: MCPToolCall, auth_context: AuthContext, request: Optional[Request]) -> MCPToolResult:
        """Handle removing a backend server."""
        
        if not auth_context.is_admin:
            await self._log_audit_event(
                auth_context, "server_remove", "servers", tool_call.arguments, "denied", "Admin required", request
            )
            raise HTTPException(status_code=403, detail="Admin access required")
        ...
        await self._log_audit_event(
            auth_context, "server_remove", f"server:{name}", tool_call.arguments, "success", None, request
        )
    ...
    async def _handle_server_update(self, tool_call: MCPToolCall, auth_context: AuthContext, request: Optional[Request]) -> MCPToolResult:
        """Handle updating a backend server."""
        
        if not auth_context.is_admin:
            await self._log_audit_event(
                auth_context, "server_update", "servers", tool_call.arguments, "denied", "Admin required", request
            )
            raise HTTPException(status_code=403, detail="Admin access required")
        ...
        await self._log_audit_event(
            auth_context, "server_update", f"server:{name}", tool_call.arguments, "success", None, request
        )
        
        return MCPToolResult(
            content=[{
                "type": "text",
                "text": f"Server '{name}' updated successfully"
            }]
        )
    
    async def _handle_servers_list(self, tool_call: MCPToolCall, auth_context: AuthContext, request: Optional[Request]) -> MCPToolResult:
    ...
    async def _handle_metrics_get(self, tool_call: MCPToolCall, auth_context: AuthContext, request: Optional[Request]) -> MCPToolResult:
        ...
        await self._log_audit_event(
            auth_context, "metrics_get", f"metrics:{metric_type}", tool_call.arguments, "success", None, request
        )
    ...
    async def _handle_health_check(self, tool_call: MCPToolCall, auth_context: AuthContext, request: Optional[Request]) -> MCPToolResult:
        """Handle health check."""
        ...
        if server_name:
            ...
            health_status = {}
            for server in self.config.servers:
                health_status[server.name] = await self.load_balancer.get_server_health(server.name)
        
        await self._log_audit_event(
            auth_context, "health_check", f"health:{server_name or 'all'}", tool_call.arguments, "success", None, request
        )
    ...
    async def _handle_users_list(self, tool_call: MCPToolCall, auth_context: AuthContext, request: Optional[Request]) -> MCPToolResult:
        """Handle listing users (admin only)."""
        
        if not auth_context.is_admin:
            await self._log_audit_event(
                auth_context, "users_list", "users", {}, "denied", "Admin required", request
            )
            raise HTTPException(status_code=403, detail="Admin access required")
        ...
        for api_key, user_id in self.config.auth.api_keys.items():
            users.append({
                ...
                "rate_limit_tier": "admin" if user_id.endswith("-admin") else "default"
            })
    ...
    async def _handle_user_create(self, tool_call: MCPToolCall, auth_context: AuthContext, request: Optional[Request]) -> MCPToolResult:
        """Handle user creation (admin only)."""
        
        if not auth_context.is_admin:
            await self._log_audit_event(
                auth_context, "user_create", "users", tool_call.arguments, "denied", "Admin required", request
            )
            raise HTTPException(status_code=403, detail="Admin access required")
        ...
        await self._log_audit_event(
            auth_context, "user_create", f"user:{user_id}", tool_call.arguments, "success", None, request
        )
    ...
    async def _handle_user_delete(self, tool_call: MCPToolCall, auth_context: AuthContext, request: Optional[Request]) -> MCPToolResult:
        """Handle user deletion (admin only)."""
        
        if not auth_context.is_admin:
            await self._log_audit_event(
                auth_context, "user_delete", "users", tool_call.arguments, "denied", "Admin required", request
            )
            raise HTTPException(status_code=403, detail="Admin access required")
        ...
        # Note: In real implementation, this would update the actual configuration
        await self._log_audit_event(
            auth_context, "user_delete", f"user:{user_id}", tool_call.arguments, "success", None, request
        )
    ...
    async def _handle_rate_limits_get(self, tool_call: MCPToolCall, auth_context: AuthContext, request: Optional[Request]) -> MCPToolResult:
        ...
        return MCPToolResult(
            content=[{
                "type": "text",
                "text": json.dumps(rate_limits, indent=2)
            }]
        )
    
    async def _handle_rate_limits_update(self, tool_call: MCPToolCall, auth_context: AuthContext, request: Optional[Request]) -> MCPToolResult:
        """Handle rate limits update (admin only)."""
        
        if not auth_context.is_admin:
            await self._log_audit_event(
                auth_context, "rate_limits_update", "rate_limits", tool_call.arguments, "denied", "Admin required", request
            )
            raise HTTPException(status_code=403, detail="Admin access required")
        
        # Note: In real implementation, this would update the actual configuration
        await self._log_audit_event(
            auth_context, "rate_limits_update", "rate_limits", tool_call.arguments, "success", None, request
        )
        
        return MCPToolResult(
            content=[{
                "type": "text",
                "text": "Rate limits updated successfully"
            }]
        )
    
    async def _handle_audit_logs_get(self, tool_call: MCPToolCall, auth_context: AuthContext, request: Optional[Request]) -> MCPToolResult:
        """Handle audit logs retrieval (admin only)."""
        
        if not auth_context.is_admin:
            await self._log_audit_event(
                auth_context, "audit_logs_get", "audit", tool_call.arguments, "denied", "Admin required", request
            )
            raise HTTPException(status_code=403, detail="Admin access required")
        ...
        if operation_filter:
            filtered_logs = [log for log in filtered_logs if operation_filter in log.operation]
        ...
        await self._log_audit_event(
            auth_context, "audit_logs_get", "audit", tool_call.arguments, "success", None, request
        )
    ...
    async def _handle_logs_get(self, tool_call: MCPToolCall, auth_context: AuthContext, request: Optional[Request]) -> MCPToolResult:
        """Handle system logs retrieval (admin only)."""
        
        if not auth_context.is_admin:
            await self._log_audit_event(
                auth_context, "logs_get", "logs", tool_call.arguments, "denied", "Admin required", request
            )
            raise HTTPException(status_code=403, detail="Admin access required")
        
        # Note: In real implementation, this would retrieve actual system logs
        mock_logs = [
            {"timestamp": "2025-06-15T10:30:00Z", "level": "INFO", "message": "MCPRelay started successfully"},
            {"timestamp": "2025-06-15T10:31:15Z", "level": "INFO", "message": "Health check passed for server-1"},
            {"timestamp": "2025-06-15T10:32:30Z", "level": "WARNING", "message": "Rate limit exceeded for user test-user"},
            {"timestamp": "2025-06-15T10:33:45Z", "level": "ERROR", "message": "Backend server connection failed"}
        ]
        
        await self._log_audit_event(
            auth_context, "logs_get", "logs", tool_call.arguments, "success", None, request
        )
        
        return MCPToolResult(
            content=[{
                "type": "text",
                "text": json.dumps(mock_logs, indent=2)
            }]
        )


def create_admin_mcp_app(config: MCPRelayConfig, load_balancer: LoadBalancer) -> FastAPI:
