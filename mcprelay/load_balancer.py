...
class LoadBalancer:
    ...
    async def get_healthy_servers(self) -> List[MCPServerConfig]:
        ...
        return [
            health.server 
            for health in self.servers.values() 
            if health.is_healthy
        ]
    
    def _filter_by_tags(self, servers: List[MCPServerConfig], path: str) -> List[MCPServerConfig]:
        ...
        if "smart-home" in path or "hue" in path:
            return [s for s in servers if "smart-home" in s.tags or "lighting" in s.tags]
    ...
    async def _check_all_servers(self):
        ...
        tasks = [
            self._check_server_health(health) 
            for health in self.servers.values()
        ]
