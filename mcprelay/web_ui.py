...
async def admin_dashboard(
    request: Request, auth_context: AuthContext = Depends(require_admin)
):
    """Admin dashboard."""
    return templates.TemplateResponse(
        "dashboard.html", {"request": request, "user": auth_context.user_id}
    )
...
async def manage_servers(
    ...
    return templates.TemplateResponse(
        "servers.html",
        {"request": request, "servers": config.servers, "user": auth_context.user_id},
    )
...
async def add_server(
    ...
    new_server = MCPServerConfig(
        name=name, url=url, timeout=timeout, weight=weight, tags=tag_list
    )
...
async def view_logs(
    ...
    return templates.TemplateResponse(
        "logs.html", {"request": request, "logs": [], "user": auth_context.user_id}
    )
...
async def metrics_dashboard(
    ...
    """Metrics dashboard."""
    return templates.TemplateResponse(
        "metrics.html", {"request": request, "user": auth_context.user_id}
    )
